...
class Controlador_tutor extends CI_Controller {
	...
	public function modificar_tutor()
	{
		...
		$id_tutor = $this->input->post("tutor_id");
		$consulta = $this->model_persona->IDtutorfPersonaUser($id_tutor);
		...
		if($this->form_validation->run() == TRUE)
		{
				...
				$nombreFoto = $foto_perfil;//"assets/images/avatar/default.png";
				if($_FILES['foto']['name'] != "")
					{
						$file = $_FILES["foto"];
						$carpetaAdjunta="assets/images/avatar/";
				...
				$this->model_tutor->modificar_tutor($id_tutor,$data);
				$verifica = true;
		}
		else {
			$verifica = false;
